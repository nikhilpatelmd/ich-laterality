...
#     axis.text.y = element_text(size = rel(0.8)),
      axis.title.y = element_text(size = rel(0.5), margin = margin(0, 4, 0, 0)),
...
bob <- ich_aggressive |>
  tabyl(neurosurgery_evac, ich_laterality) |>
...
ich_aggressive |>
  tabyl(days_mechanical_ventilation, ich_laterality)
...
```{r table 2}
#| label: tbl-2
#| tbl-cap: "Association between hemispheric lateralization and aggressive care"
#| echo: false
#| warning: false

# gather odds ratios and 95% credible intervals for each model
...
tibble_2 |>
  ...
  tab_footnote(
    footnote = "Adjusted for: age, admission GCS, ICH location, IVH, and study (as random intercept)",
    locations = cells_column_labels(columns = 4)
  ) |>
  tab_footnote(
    footnote = "Region of Practical Equivalence, defined as 0.95 > aOR > 1.05",
...
predictions(models$"Neurosurgical Intervention", by = "ich_laterality") |>
  posterior_draws() |>
...
predictions(models$"Neurosurgical Intervention", by = c("ich_laterality", "ich_location")) |>
  posterior_draws() |>
  ggplot(aes(x = draw, fill = ich_laterality)) +
  geom_density(alpha = .7) +
