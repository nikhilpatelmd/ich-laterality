...
erich.whole$nsgy.evacuation.date <- round(erich.whole$nsgy.interval, digits = 0) # Makes it compatabible to merge with ATACH
...
clear3.whole <- clear3.whole %>%
  ...
  rename(comfort.binary = withdrawal_contribution) %>%
  rename(comfort.date.time = ictus_to_withdrawal_days) %>%
  rename(symptom.onset.to.presentation = ictus_to_presentation)
...
mistie2.combined.demographics <- full_join(mistie2.demographics, mistie2.demographics3)
mistie2.combined.demographics <- full_join(mistie2.combined.demographics, mistie2.demographics2)
...
mistie3 <- mistie3 %>%
  ...
  rename(time.to.presentation = ictus_to_site_arrival_hours)

mistie3 <- mistie3 %>%
  mutate(race = case_when(
    race == "African-American" ~ "Black of African-American",
...
table1 <- CreateTableOne(data = atach.erich,
