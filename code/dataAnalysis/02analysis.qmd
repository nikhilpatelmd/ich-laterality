...
mrsCombined <-
  (gmrs90 + theme(plot.margin = unit(c(50, 0, 75, 0), "pt"))) / (gmrs180 + theme(plot.margin = unit(c(0, 0, 75, 0), "pt"))) / (gmrs365 + theme(plot.margin = unit(c(0, 0, 75, 0), "pt"))) +
    plot_annotation(
      title = "ICH Laterality and modified Rankin Scale",
...
euroQolCombined90 <-
  ...
  plot_annotation(
    title = "ICH Laterality and HRQOL at 90 days",
    theme = theme(plot.title = element_text(face = "bold", size = 35, lineheight = 0.1))
  )
# plot_layout(guides = 'collect') & theme(legend.position='right', legend.direction="vertical")
...
euroQolCombined180 <-
  ...
  plot_annotation(
    title = "ICH Laterality and HRQOL at 180 days",
    theme = theme(plot.title = element_text(face = "bold", size = 35, lineheight = 0.1))
  )
# plot_layout(guides = 'collect') & theme(legend.position='right', legend.direction="vertical")
...
euroQolCombined365 <-
  ...
  plot_annotation(
    title = "ICH Laterality and HRQOL at 365 days",
    theme = theme(plot.title = element_text(face = "bold", size = 35, lineheight = 0.1))
  )
# plot_layout(guides = 'collect') & theme(legend.position='right', legend.direction="vertical")
...
ggeuroVASCombined <-
  ...
  plot_annotation(
    title = "ICH Laterality EuroQOL VAS",
    theme = theme(plot.title = element_text(face = "bold", size = 35, lineheight = 0.1))
  )
...
Let's define functional outcome as EuroQOL VAS, an outcome measure from 0-100.

```{r}
d |>
  ggplot(aes(x = euroVAS90, fill = ichLaterality)) +
  geom_density(alpha = 0.5)

eurovas90Model <- stan_glm(
  euroVAS90 ~ ichLaterality,
  data = d,
  family = gaussian,
  iter = 5000,
  warmup = 2000,
...
summary(eurovas90Model)

tidy(eurovas90Model,
  effects = c("fixed", "aux"),
  conf.int = TRUE, conf.level = 0.89
) |>
  select(-std.error)

# MCMC diagnostics

mcmc_trace(eurovas90Model, size = 0.1)
mcmc_dens_overlay(eurovas90Model)
mcmc_acf(eurovas90Model)

## Posterior predictive checks

pp_check(eurovas90Model)

## Posterior models

head(as.data.frame(eurovas90Model))
